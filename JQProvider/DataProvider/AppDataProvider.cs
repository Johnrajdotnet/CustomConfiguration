...
namespace JQProvider.DataAcess.DataProvider
{
    ...
    {
        protected class AppDataProvider: DataContractProvider
        {
            ...
            public override T QuerySingle<T>(string commandName, DynamicParameters parameters)
            {
                try
                {
            ...
            /// <param name="parameters"></param>
            /// <returns></returns>
            public override int QueryExecuteTransaction(string commandName, DynamicParameters parameters)
            {
                try
                {
                    using (var connection = SqlConnectionFactory.CreateConnection(ConnectionString))
            ...
            public override IList<T> QueryList<T>(string commandName, DynamicParameters parameters)
            {
                try
                {
                    using (var connection = SqlConnectionFactory.CreateConnection(ConnectionString))
            ...
            /// <param name="parameters"></param>
            /// <returns> </returns>
            public override int CommandExecute<T>(string commandName, DynamicParameters parameters)
            {
                try
                {
                    using (var connection = SqlConnectionFactory.CreateConnection(ConnectionString))
            ...
            /// <param name="parameters"></param>
            /// <returns> </returns>
            public override int CommandExecuteMutipleRow<T>(string commandName, List<DynamicParameters> parameters)
            {
                try
                {
                    using (var connection = SqlConnectionFactory.CreateConnection(ConnectionString))
            ...
            public override T ExecuteScalarFunc<T>(string commandName, DynamicParameters parameters)
            {
                try
                {
            ...
            /// <param name="parameters"></param>
            /// <returns>Returns a  list of  records </returns>
            public override IList<T> ExecuteTableFunc<T>(string commandName, DynamicParameters parameters)
            {
                try
                {
            ...
            /// <param name="parameters"></param>
            /// <returns>Returns a single Record </returns>
            public override async Task<T> QuerySingleAsync<T>(string commandName, DynamicParameters parameters)
            {
                try
                {
                    using (var connection = SqlConnectionFactory.CreateConnection(ConnectionString))
            ...
            /// <param name="parameters"></param>
            /// <returns>Returns a  list of  records </returns>
            public override async Task<IList<T>> QueryListAsync<T>(string commandName, DynamicParameters parameters)
            {
                try
                {
                    using (var connection = SqlConnectionFactory.CreateConnection(ConnectionString))
            ...
            /// <param name="parameters"></param>
            /// <returns></returns>
            public override async Task<int> CommandExecuteAsync<T>(string commandName, DynamicParameters parameters)
            {
                try
                {
                    using (var connection = SqlConnectionFactory.CreateConnection(ConnectionString))
            ...
            /// <param name="commandName"></param>
            /// <param name="parameters"></param>
            public override DynamicMutipleSetModel<T,K> QueryMultipleSetAsync<T, K>(string commandName, DynamicParameters parameters)
            {
                using (var connection = SqlConnectionFactory.CreateConnection(ConnectionString))
                {
            ...
            /// <param name="parameters"></param>
            /// <returns></returns>
            public override DynamicMutipleSetModel<K, M, N> QueryMultipleSetAsync<K, M, N>(string commandName, DynamicParameters parameters)
            {
                using (var connection = SqlConnectionFactory.CreateConnection(ConnectionString))
                {
                    using (Dapper.SqlMapper.GridReader multi = connection.QueryMultipleAsync(commandName, parameters, commandType: CommandType.StoredProcedure).Result)
            ...
            /// <param name="parameters"></param>
            /// <returns></returns>
            public override DynamicMutipleSetModel<M, N, O, P, Q, R> QueryMultipleSetAsync<M, N, O, P, Q, R>(string commandName, DynamicParameters parameters)
            {
                using (var connection = SqlConnectionFactory.CreateConnection(ConnectionString))
                {
                    using (Dapper.SqlMapper.GridReader multi = connection.QueryMultipleAsync(commandName, parameters, commandType: CommandType.StoredProcedure).Result)
                    {
                        ...
                        return new DynamicMutipleSetModel<M, N, O, P, Q, R>()
                        {
                            DynamicListResultSetM = resultSetM,
                            DynamicListResultSetN = resultSetN,
                            DynamicListResultSetO = resultSetO,
                            DynamicListResultSetP = resultSetP,
            ...
            /// <param name="parameters"></param>
            /// <returns></returns>
            public override DynamicMutipleModel<T, K> QueryMultipleAsync<T, K>(string commandName, DynamicParameters parameters)
            {
                using (var connection = SqlConnectionFactory.CreateConnection(ConnectionString))
                {
                    using (Dapper.SqlMapper.GridReader multi = connection.QueryMultipleAsync(commandName, parameters, commandType: CommandType.StoredProcedure).Result)
            ...
            /// <returns></returns>
            public override T QueryMultipleLevel<T>(string commandName, DynamicParameters parameters, List<MapperConfigurationIdentifiersSP> mapperIdentifiers)
            {
                using (var connection = SqlConnectionFactory.CreateConnection(ConnectionString))
                {
